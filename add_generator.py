...
checkpointer = ModelCheckpoint('add_genorator.h5', verbose=1, save_best_only=True)
results = model.fit_generator(train_generator, steps_per_epoch=len(xtr)/6, epochs=35,
                        validation_data=val_generator, validation_steps=len(xval)/batch_size, callbacks=[earlystopper, 	
			checkpointer])

print('train is done')

# Predict on train, val and test
model = load_model('add_genorator.h5', custom_objects={'mean_iou': mean_iou})
...
sub.to_csv('add_generator.csv', index=False)
